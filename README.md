...
**Resolution changes pixel dimensions only.** It defaults to 0.25× CSS resolution with dither; lower values produce crisp pixel blocks. Ray sampling, disk detail, and glow radius stay fixed. Disk texture strength defaults to 105%, with flowing knots and shaded spiral ridges; set it to zero for a smooth disk.
