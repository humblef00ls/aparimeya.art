...
**Resolution changes pixel dimensions only.** It defaults to 0.33× CSS resolution with dither (Dither scale 8, ASCII scale 7); lower values produce crisp pixel blocks. Ray sampling, disk detail, and glow radius stay fixed. Disk texture strength defaults to 105%, with flowing knots and shaded spiral ridges; set it to zero for a smooth disk.
